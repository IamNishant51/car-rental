function updateNavbar() {
  ...
  let html = `
    <a href="${p('/')}" class="${window.location.pathname === p('/') || window.location.pathname === BASE + '/index.html' ? 'active' : ''}">Home</a>
    <a href="${p('/pages/vehicles.html')}" class="${window.location.pathname.includes('vehicles') ? 'active' : ''}">Vehicles</a>
  `;

  if (isAuth) {
    html += `<a href="${p('/pages/booking.html')}">My Bookings</a>`;
    if (isAdmin) {
...
function requireAuth() {
  if (!api.isAuthenticated()) {
    window.location.href = p('/pages/login.html?redirect=') + encodeURIComponent(window.location.pathname);
    return false;
  }
  return true;
}
